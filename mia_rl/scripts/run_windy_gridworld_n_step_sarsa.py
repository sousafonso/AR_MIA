...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Windy Gridworld with n-step Sarsa control.")
    parser.add_argument("--episodes", type=int, default=500, help="Number of training episodes.")
    parser.add_argument("--n-steps", type=int, default=4, help="Number of steps used in the return target.")
    parser.add_argument("--alpha", type=float, default=0.5, help="n-step Sarsa step-size.")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/windy_gridworld_n_step_sarsa",
...
def main() -> None:
    ...
    from mia_rl.agents.control import NStepSarsaControl
    ...
    try:
        ...
        fig_lengths, _ = plot_episode_lengths(episode_lengths, title=f"{args.n_steps}-step Sarsa: episode length over training")
        fig_rewards, _ = plot_episode_rewards(episode_rewards, title=f"{args.n_steps}-step Sarsa: episode reward over training")
        fig_policy, _ = plot_policy(env, policy, path=path, title=f"Windy Gridworld greedy policy after {args.n_steps}-step Sarsa training")
        ...
        print("\nThis optional extension depends on the main practical being complete first.")
        print("Please finish the TODOs in:")
        print("- mia_rl/envs/windy_gridworld.py")
        print("- mia_rl/agents/control/sarsa.py")
        print("- mia_rl/agents/control/n_step_sarsa.py")
