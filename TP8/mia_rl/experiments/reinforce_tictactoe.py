...
def run_reinforce_episode(env: TicTacToeEnv, agent: ReinforceAgent) -> tuple[float, int]:
    ...
    while not env.is_terminal(state):
        ...
        if done and reward == 1.0:
            if player == 1 and traj_o:
                last_phi, last_action, last_available, _ = traj_o[-1]
                traj_o[-1] = (last_phi, last_action, last_available, -1.0)
            elif player == -1 and traj_x:
                last_phi, last_action, last_available, _ = traj_x[-1]
                traj_x[-1] = (last_phi, last_action, last_available, -1.0)
